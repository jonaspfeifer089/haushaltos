...
export async function GET(request: Request) {
  ...
  try {
    ...
    temp = `${Math.round(weatherData?.current?.temperature_2m ?? 0)}°C`;
  } catch (e: any) {
  ...
  // 3. iCloud Kalender Termine heute
  ...
  // 4. Personalisierte Nachricht erstellen
  ...
  try {
    if (targetUser === "Jonas") {
      await fetch("https://ntfy.sh/HaushaltJonas", {
        ...
        headers: { "Title": "Haushalt OS - Briefing Jonas", "Tags": "sunrise,sparkles", "Priority": "default" }
      });
    } else if (targetUser === "Lena") {
      await fetch("https://ntfy.sh/HaushaltLena", {
        method: "POST",
        body: createBody("Lena"),
        headers: { "Title": "Haushalt OS - Briefing Lena", "Tags": "sunrise,sparkles", "Priority": "default" }
      });
    } else {
      await Promise.all([
        fetch("https://ntfy.sh/HaushaltJonas", {
          method: "POST",
          body: createBody("Jonas"),
          headers: { "Title": "Haushalt OS - Guten Morgen Jonas", "Tags": "sunrise,clipboard", "Priority": "default" }
        }),
        fetch("https://ntfy.sh/HaushaltLena", {
          method: "POST",
          body: createBody("Lena"),
          headers: { "Title": "Haushalt OS - Guten Morgen Lena", "Tags": "sunrise,clipboard", "Priority": "default" }
        })
      ]);
    }
